...
FabricPool の使用に関する一般的な情報については、を参照してください link:https://docs.netapp.com/us-en/ontap/disks-aggregates/index.html["ディスクと階層（アグリゲート）の管理"^]。
...
NOTE: アグリゲートを設定する前に、の説明に従ってオブジェクトストアを設定する必要があります。 link:https://docs.netapp.com/us-en/ontap/fabricpool/setup-object-stores-mcc-task.html["MetroCluster 構成で FabricPool のオブジェクトストアを設定する"^]。
...
ONTAP 9.11.1以降、 link:https://docs.netapp.com/us-en/ontap/consistency-groups/index.html["整合グループ"^] MetroCluster 構成でサポートされます。
...
+
image::../media/svm_dr_image_2.gif[スイッチオーバー後のSVM災害復旧]
