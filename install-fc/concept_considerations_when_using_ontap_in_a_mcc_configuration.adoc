...
xref:concept_considerations_when_using_ontap_in_a_mcc_configuration.adoc[MetroCluster 構成での LIF の作成に関する要件]

xref:concept_considerations_when_using_ontap_in_a_mcc_configuration.adoc[LIF のレプリケーションおよび配置の要件と問題]
...
xref:concept_considerations_when_using_ontap_in_a_mcc_configuration.adoc[IPspace オブジェクトのレプリケーションとサブネットの設定の要件]
...
MetroCluster check lif show コマンドで表示される LIF 配置エラーは ' スイッチオーバー操作の後も保持されます配置エラーがある LIF に対して network interface modify 、 network interface rename MetroCluster 、または network interface delete のいずれかのコマンドを実行すると、エラーが削除され、「 lif check show 」コマンドの出力に表示されなくなります。
...
の「 SVM の設定のレプリケート」セクションを参照してください link:http://docs.netapp.com/ontap-9/topic/com.netapp.doc.pow-dap/home.html["データ保護パワーガイド"] SVM DR 関係の設定の詳細については、を参照してください。
