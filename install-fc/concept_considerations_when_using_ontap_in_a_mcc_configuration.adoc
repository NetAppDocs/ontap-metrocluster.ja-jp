...
NOTE: アグリゲートを設定する前に、 FabricPool の「 MetroCluster 用のオブジェクトストアのセットアップ」の説明に従ってオブジェクトストアを設定する必要がありますhttps://["ディスクとアグリゲートの管理"]
...
. の説明に従って、アグリゲートを使用して FabricPool ミラーを作成します https://["ディスクとアグリゲートの管理"]
+
...
ONTAP 9.11.1以降、 https://["整合グループ"^] MetroCluster 構成でサポートされます。
