...
summary: ノード レベルの構成や 2 つのサイトへのノードの構成など、 ONTAPでMetroCluster構成内の各ノードを設定する必要があります。 
---
...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[高レベルのノードとクラスタ構成のワークフロー]
...
NOTE: EngineID の場合は、*ENTER* を押してデフォルト値を割り当てます。
...
NOTE: また、未使用のアカウントにパスワードを設定して保護し、 ONTAP リリースで最適な暗号化機能を使用する必要があります。
...
NOTE: セキュリティを強化するために、NetAppでは、スイッチのヘルスを監視するようにSNMPv2またはSNMPv3を設定することを推奨します。
...
* 健全性監視に専用のネットワークを使用する場合は、各ノードのノード管理LIFをその専用のネットワーク内に配置する必要があります。
