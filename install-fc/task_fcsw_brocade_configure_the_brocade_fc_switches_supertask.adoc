...
+
xref:concept_tdm_wdm.adoc[ファブリック接続 MetroCluster 構成で TDM / WDM 機器を使用する場合の考慮事項]
...
. スイッチを永続的に無効にします。「 witchcfgpersistentdisable
+
これにより、リブート後や高速ブート後もスイッチが無効なままになります。このコマンドが使用できない場合は、 switchdisable コマンドを使用します。
...
BrocadeSwitchA:admin> switchcfgpersistentdisable
----
. 「 witchname switch_name 」と入力して、スイッチ名を設定します。
+
...
FC_switch_B_1:admin>
----
. ポートごとに次のコマンドを実行して、すべてのポートをデフォルト値に設定します。 `portcfgdefault`
+
...
----
. 次のコマンドを実行して ' ゾーニング情報を消去します cfgdisable cfgclear cfgsave
+
次の例は、 FC_switch_A_1 に対するコマンドを示しています。
...
----
. スイッチの一般的な設定をデフォルトの「 configdefault 」に設定します
+
次の例は、 FC_switch_A_1 に対するコマンドを示しています。
...
----
. すべてのポートを非トランキングモードに設定します : 'witchcfgtrunk 0
+
次の例は、 FC_switch_A_1 に対するコマンドを示しています。
...
----
. Brocade 6510 スイッチで、 Brocade Virtual Fabric （ VF ）機能を無効にします。「 fosconfig options
+
次の例は、 FC_switch_A_1 に対するコマンドを示しています。
...
... iodshow 、 aptpolicy 、および `d lsshow の各コマンドを使用して OOD 設定を検証します。
+
たとえば、 FC_switch_A_1 で次のコマンドを問題に実行します。
...
xref:concept_prepare_for_the_mcc_installation.adoc[Brocade DCX 8510-8 スイッチを使用するための要件]
...
|===
.手順
. [step1-brocad-config 、 Step 1] ポート速度を設定します。
+
...
3: fffc03 10:00:00:05:33:8c:2e:9a 10.10.10.65  0.0.0.0    >"FC_switch_B_1
----
. [[step10-brocade-config, Step 10] ISL のトランキングを確認します。
+
「 trunkshow 」
...
. 繰り返します <<step1-brocade-config>> から <<step10-brocade-config>> 2 つ目の FC スイッチファブリック。
...
xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[FC-VI ポートのゾーニング]

xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[FibreBridge 6500N ブリッジまたは 1 つの FC ポートを使用する FibreBridge 7500N / 7600N ブリッジのゾーニング]

xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[両方の FC ポートを使用する FibreBridge 7500N ブリッジのゾーニング]
...
.. 必要に応じて、設定にゾーンを追加します。
+
「 +cfgadd config_name zone ； zone...+` 」のように設定します
