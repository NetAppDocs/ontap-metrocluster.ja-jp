...
ノード 1 IC LIF 2
 ...
 a| 
2 ノード MetroCluster 構成（サイトごとに 1 つのノード）の場合、ノード 2 IC LIF 1 は不要です。
 a| 
 a| 
 a| 
 a| 
...
| CNA FC | ucadmin modify -m fc -t initiator_adapter_name _ ` 


| CNA イーサネット | ucadmin modify -mode cna_adapter_name _ ` 


| FC ターゲット | fcadmin config -t target_adapter_name _` 


| FC イニシエータ | fcadmin config -t initiator_adapter_name_` 
|===
. メンテナンスモードを終了します。
+
「 halt 」
...
+
.. 最初のノードで、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「 disk assign -shelf local-switch-name ： shelf-name. port -p pool
+
...
----
.. ローカルサイトの 2 番目のノードに対して処理を繰り返し、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「ディスク割り当てシェルフローカルスイッチ名： shelf-name .port-p pool 」
+
...
+
.. リモートサイトの最初のノードで、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「 disk assign -shelf local-switch-namesshelf-name -p pool
+
...
----
.. リモートサイトの 2 番目のノードに対して処理を繰り返し、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「ディスク割り当てシェルフシェルフ名 -p pool 」
+
...
. シェルフの割り当てを確認します。「 storage show shelf
. 保守モードを終了します :halt
...
+
.. 最初のノードで、各シェルフの半分のディスクをプール 0 に、残りの半分を HA パートナーのプール 0 に割り当てます。「 disk assign -disk disk-name -p pool -n number-of-disks 」
+
...
----
.. ローカルサイトの 2 番目のノードに対して処理を繰り返し、各シェルフの半分のディスクをプール 1 に、残りの半分を HA パートナーのプール 1 に割り当てます。「ディスクの割り当てディスク名 -p pool 」
+
ストレージコントローラ Controller_A_1 にシェルフが 4 台あり、各シェルフに SSD が 8 本ある場合は、次のコマンドを問題に設定します。
...
+
.. リモートサイトの最初のノードで、各シェルフの半分のディスクをプール 0 に、残りの半分を HA パートナーのプール 0 に割り当てます。「ディスクの割り当てディスク名 -p pool 」
+
...
----
.. リモートサイトの 2 番目のノードに対して処理を繰り返し、各シェルフの半分のディスクをプール 1 に、残りの半分を HA パートナーのプール 1 に割り当てます。「 disk assign -disk disk-name -p pool 」
+
...
. ディスクの割り当てを確認します。「 storage show disk 」
. 保守モードを終了します :halt
. ブートメニューを表示します：「 boot_ontap menu
. 各ノードで、オプション * 4 * を選択してすべてのディスクを初期化します。
...
. システムが工場から届いた場合は、「 Disk show – v 」というシェルフ割り当てを確認します
...
+
.. システムをブートしていない場合は、メンテナンスモードでブートします。
.. サイト A のノードで、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「 disk assign -shelf disk_shelf_name -p pool
+
...
----
.. リモートサイト（サイト B ）のノードで、ローカルディスクシェルフをプール 0 に、リモートディスクシェルフをプール 1 に割り当てます。「ディスク割り当て -shelf disk_shelf_name -p pool 」
+
...
----
.. 各ディスクのディスクシェルフ ID とベイを表示します。「 disk show – v 」
...
. クラスタセットアップウィザードが完了したら、次のコマンドを入力して、クラスタがアクティブで、最初のノードが正常に機能していることを確認します。「 cluster show
+
...
+
システムが保守モードになっている場合は問題、 halt コマンドを使用して保守モードを終了し、 LOADER プロンプトから問題コマンドを実行します。「 boot_ontap
...
+
そうでない場合は、各ノードで次のコマンドを問題処理してノードをリブートする必要があります：「 storage failover modify -mode ha -node localhost 」
...
xref:concept_prepare_for_the_mcc_installation.adoc[専用のポートを使用する場合の考慮事項]

xref:concept_prepare_for_the_mcc_installation.adoc[データポートを共有する場合の考慮事項]
...
+
.. データ保護のデスティネーションクラスタで、データ保護のソースクラスタとのピア関係を作成します。「 cluster peer create -peer-addrs peer_lif_ips-ipspace ip`` 」
+
カスタム IPspace を使用しない場合は、 -ipspace オプションを無視してかまいません。コマンド構文全体については、マニュアルページを参照してください。
...
.. データ保護のソース・クラスタで ' ソース・クラスタをデスティネーション・クラスタに対して認証します「 cluster peer create -peer-addrs peer_lif_ips-ipspace ips'
+
コマンド構文全体については、マニュアルページを参照してください。
...
|===
+
* 注： * 複数のデータアグリゲートを使用することを推奨します。最初の DR グループにアグリゲートが 1 つしかなく、 1 つのアグリゲートを含む DR グループを追加する場合は、メタデータボリュームを単一のデータアグリゲートから移動する必要があります。この手順の詳細については、を参照してください http://docs.netapp.com/ontap-9/topic/com.netapp.doc.hw-metrocluster-service/GUID-114DAE6E-F105-4908-ABB1-CE1D7B5C7048.html["MetroCluster 構成でのメタデータボリュームの移動"]。
...
+
... advanced モードに切り替えます。「 set advanced
... 設定を確認します MetroCluster interconnect adapter show
+
[listing]
----
...
+
... advanced モードに切り替えます。「 set advanced
... MetroCluster 構成の設定を確認します MetroCluster interconnect adapter show
+
[listing]
----
mcc4-b12_siteB::*> metrocluster interconnect adapter show
...
8 entries were displayed.
----
... ノード「 `m cc4-b1' 」およびノード「 `m cc4-b1' 」で OOD を有効にします。 MetroCluster interconnect adapter modify -node node name -is-ood -enabled true
+
[listing]
----
...
. コントローラプロンプトから各スイッチの SNMP ユーザを作成します。「 securitylogin create 」
+
[listing]
----
...
10.10.11
----
. スイッチごとに 1 つのログインエントリがあることを確認します。 'securitylogin show
+
[listing]
----
...
----
. スイッチのプロンプトで、 SNMPv3 を設定します。「 mpconfig --set snmpv3 」
+
...
NOTE: ONTAP 9.8 以降では、「 storage switch 」コマンドは「 system switch 」コマンドに置き換えられています。次の手順は「 storage switch 」コマンドを示していますが、 ONTAP 9.8 以降を実行している場合は「 system switch 」コマンドを使用することを推奨します。

. 各 MetroCluster ノードに IP アドレスを指定してスイッチを追加します。「 storage switch add -address ipaddress
+
...
----
. すべてのスイッチが正しく設定されていることを確認します。「 storage switch show
+
...
+
.... ストレージフェイルオーバーが有効になっていることを確認します。「 storage failover show
+
...
----
.... controller_A_1 から controller_A_2 をテイクオーバーします。「 storage failover takeover controller_A_2 」
+
storage failover show-takeover コマンドを使用すると、テイクオーバー処理の進捗を監視できます。

.... テイクオーバーが完了したことを確認します。「 storage failover show
+
...
2 entries were displayed.
----
.... controller_A_2 をギブバックします。「 storage failover giveback controller_A_2
+
storage failover show-giveback コマンドを使用すると、ギブバック処理の進捗を監視できます。

.... ストレージフェイルオーバーが通常の状態に戻ったことを確認します。「 storage failover show
+
両方のノードでテイクオーバーが可能であることが出力に示されます。
