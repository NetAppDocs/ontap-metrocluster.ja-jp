...
. 次のコマンドを使用して ISL ポートごとの距離を設定します。 *portcfglongdistance * _port levelvc_link_initdistance_
+
vc_link_init の値が 1 の場合は、デフォルトで fillword 「 ARB 」が使用されます。値 0 は fillword 'idle' を使用します。必要な値は、使用するリンクによって異なる場合があります。この例では、デフォルトが設定され、距離は 20 km と想定されていますしたがって、設定は「 30 」で、 vc_link_init の値は 1 、 ISL ポートは 21 です。
...
----
. IP アドレスが正しく設定されているかどうかを確認します :`*ipaddrshow*`
+
[listing]
----
...
----
+
必要に応じて次のコマンドで IP アドレスを設定できます :`*ipAddrSet*`

. スイッチのプロンプトからタイムゾーンを設定します :`*timezone --interactive *`
+
プロンプトに対してと入力します。
...
+
xref:task_reset_the_brocade_fc_switch_to_factory_defaults.adoc[Brocade FC スイッチを工場出荷時のデフォルトにリセット]

. 構成を検証
+
.. スイッチが 1 つのファブリックを形成していることを確認します :'*switchshow*
+
...
----
.. ISL が機能していることを確認します。「 * islshow * 」
+
[listing]
----
FC_switch_A_1:admin> islshow
----
.. 次のコマンドを実行して ' ゾーニングが正しく複製されていることを確認します +`*cfgshow*`+`*zoneshow *`
+
