...
NOTE: スイッチがすでに正しく設定されている場合は、以降のセクションを省略して、に直接進むことができます <<Configuring MACsec encryption on Cisco 9336C switches>>（該当する場合）またはに移動します <<Connecting the new NS224 shelf>>。
