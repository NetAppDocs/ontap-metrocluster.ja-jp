...
[role="lead"]
メンテナンスまたは移転を開始する前に、MetroCluster IP 構成全体とすべての機器の電源をオフにします。
...
NOTE: MetroCluster IP構成では、構成でONTAP Mediatorを使用していない限り、AUSO障害ドメインはすでに「auso-disabled」に設定されています。
