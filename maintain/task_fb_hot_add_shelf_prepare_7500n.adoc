...
NOTE: この手順では、リンク先に記載されている手順のダウンロード部分のみを実行します。の手順に従う必要があります xref:task_fb_hot_add_a_disk_shelf_install_7500n.adoc[ディスクシェルフのホットアド] ディスクシェルフの設置については、セクションを参照してください。
