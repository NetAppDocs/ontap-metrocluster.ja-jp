...
NOTE: どちらのクラスタからも MetroCluster IP インターフェイスを設定できます。また、ONTAP 9.9..1以降では、レイヤ3設定を使用している場合は、も指定する必要があります `-gateway` MetroCluster IPインターフェイスを作成するときにパラメータを指定します。を参照してください link:../install-ip/concept_considerations_layer_3.html["レイヤ 3 ワイドエリアネットワークに関する考慮事項"]
...
.手順
. MetroCluster は、データアグリゲートが1つであるか複数であるかに応じて設定します。
+
|===
