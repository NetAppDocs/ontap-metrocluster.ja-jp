...
.. 移行モードを有効にする : MetroCluster transition enable-transition-mode non-disruptive
+
...
. 各サイトで、クラスタピアリングが構成されていることを確認します。「 cluster peer show
+
...
....
. MetroCluster IP ノードの DR グループを構成します MetroCluster 構成設定 DR-group create -partner-cluster
+
....
...
cluster_A::>
....
. DR グループが作成されたことを確認します。「 MetroCluster configurion-settings dr-group show 」を参照してください
+
....
...
2     cluster_B
              ...
              node_B_2-IP      ready to configure
                                                -         -
....
. 新しく加入した MetroCluster IP ノードの MetroCluster IP インターフェイスを構成します。「 MetroCluster configurion-settings interface create -cluster-name 」
+
を参照してください link:../install-ip/task_sw_config_configure_clusters.html#configuring-and-connecting-the-metrocluster-ip-interfaces["MetroCluster IP インターフェイスの設定と接続"] IP インターフェイスを設定する際の考慮事項については、を参照して注： MetroCluster IP インターフェイスは、どちらのクラスタからも設定できます。また、 ONTAP 9.9..1 以降では、レイヤ 3 設定を使用している場合、 MetroCluster IP インターフェイスを作成するときに「 -gateway 」パラメータも指定する必要があります。リンク： ./install-ip/concept_consideration_layer_3.html を参照してください。
...
....
. MetroCluster IP インターフェイスが作成されたことを確認します。「 MetroCluster configurion-settings interface show 」
+
....
...
cluster_A>
....
. MetroCluster IP インターフェイスを接続します。「 MetroCluster configuration - settings connection connect 」
+
...
cluster_A::>
....
. ディスクの自動割り当てとパーティショニングを確認します : 「 disk show -pool Pool1 」
+
....
...
cluster_A::>
....
. ルートアグリゲートをミラーします。「 storage aggregate mirror -aggregate aggr0_cluster1_01 _3-IP 」
+
...
cluster_A::>
....
. ルートアグリゲートがミラーされていることを確認します。「 storage aggregate show
+
....
cluster_A::> aggr show
...
. 新しい各 MetroCluster ノードにミラーされたデータアグリゲートを作成します。「 storage aggregate create -aggregate aggregate-name -node node-name -diskcount no-of disks - mirror true 」
+
...
cluster_A::>
....
. 変更を実装するように MetroCluster を構成します MetroCluster configure
+
....
...
cluster_A::*>
....
. ノードが DR グループに追加されていることを確認します MetroCluster node show
+
....
cluster_A::*> metrocluster node show
...
....
.. advanced 権限レベルを設定します。「 set -privilege advanced 」
.. MDV ボリュームを 1 つずつ移動します。 'volume move start -volume MDV_volume-destination-aggregate aggr-on -new-node-vserver -name'
+
...
....
.. volume show コマンドを使用して、 MDV ボリュームが正常に移動されたことを確認します。「 volume show MDV_name 」
+
