...
無停止
 a| 
新しいプラットフォームには新しいストレージ シェルフが必要です。移行が完了すると、古いコントローラー、シェルフ、ディスクはクラスターから削除されます。
 a| 
link:concept_nondisruptively_transitioning_from_a_four_node_mcc_fc_to_a_mcc_ip_configuration.html["手順 へのリンク"]

*注:* この手順では、次の FC から IP への移行がサポートされます。

* 4ノードMetroCluster FC構成から4ノードMetroCluster IP構成へ
* 8ノードのMetroCluster FC構成から8ノードのMetroCluster IP構成へ
