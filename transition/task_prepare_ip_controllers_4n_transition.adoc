...
. に示すように、 MetroCluster IP ノードを IP スイッチにケーブル接続します link:../install-ip/task_cable_ip_switches.html["IP スイッチのケーブル接続"]。
. MetroCluster インストールおよび設定ガイドの以降のセクションを使用して、 MetroCluster IP ノードを設定します。 _
+
.. link:../install-ip/task_sw_config_gather_info["必要な情報の収集"]
.. link:../install-ip/task_sw_config_restore_defaults["コントローラモジュールでの設定の消去"]
.. link:../install-ip/task_sw_config_verify_haconfig["コンポーネントの ha-config 状態の確認"]
.. link:../install-ip/task_sw_config_assign_pool0["プール 0 ドライブの手動割り当て（ ONTAP 9.4 以降）"]
