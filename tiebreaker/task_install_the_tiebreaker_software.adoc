...
自動起動機能を有効にする方法は、 MySQL のドキュメントを参照してください。
...
.手順
. 「 NetApp-MetroCluster-Tiebreaker -Software-1.3-1.x86_64 」ファイルをダウンロードします。
+
...
既存のインストールをアップグレードする
 a| 
「 rpm -Uvh NetApp-MetroCluster-Tiebreaker -Software-1.3-1.x86_64 」
...
アップグレード前にモニタをオブザーバーモードに切り替えても、 Tiebreaker モニタが実行されているホスト o を最小限の中断でアップグレードできます。
