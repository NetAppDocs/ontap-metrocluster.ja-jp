...
---
= MetroCluster Tiebreaker 1.7 をインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
MetroCluster構成を監視するには、Linux ホストに Tiebreaker 1.7 をインストールするか、アップグレードします。

.このタスクについて
* ストレージシステムでONTAP 9.12.1以降が実行されている必要があります。
* Tiebreaker 1.6P1 からのみ Tiebreaker 1.7 にアップグレードできます。参照link:tb-16-install.html["Tiebreaker 1.6または1.6P1をインストールする"]。
* MetroCluster Tiebreakerは、Tiebreakerのインストール、テーブルとユーザの作成、ユーザパスワードの設定に必要な管理権限を持つroot以外のユーザとしてインストールできます。



NOTE: Tiebreaker 1.7 をインストールしたら、シームレスな監視を有効にするためにlink:concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["証明書をインポートする"]必要があります。

.手順
. MetroCluster Tiebreaker 1.7ソフトウェアをダウンロードします。
+
...
. アップグレードする場合は、実行している Tiebreaker のバージョンを確認してください。
+
次の例はタイブレーカー1.6P1を示しています
...
--
Tiebreaker 1.7を新規にインストールする場合は、次の手順を実行します。
...
.1.6P1を1.7にアップグレード
--
Tiebreaker 1.6P1 ソフトウェア バージョンを Tiebreaker 1.7 にアップグレードするには、次の手順に従います。
...
NOTE: 証明書の不足によりアップグレードがエラーで失敗する場合は、link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["証明書をインポートする"] 。証明書をインポートしたら、再度アップグレードを試みることができます。
