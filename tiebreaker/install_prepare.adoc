...
.次の手順
* Tiebreaker 1.5以降をインストールまたはアップグレードする場合は、link:install_security.html["Tiebreakerのホストとデータベースのインストールを保護する"]。
* Tiebreaker 1.4 をインストールまたはアップグレードする場合は、link:install-dependencies-14.html["MetroCluster Tiebreaker 1.4の依存関係のインストール"]。
