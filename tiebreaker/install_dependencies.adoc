...
Tiebreakerソフトウェアをインストールまたはアップグレードする前に、ホストシステムにJREをインストールする必要があります。Tiebreaker 1.4以前を実行しているシステムの場合は、JRE 8を実行します。Tiebreaker 1.5以降を実行しているシステムの場合は、OpenJDK 17、18、または19を実行します。


NOTE: 次の例の出力はJRE 1.8.0を示しています。（JRE 8）。
...
. インストールするTiebreakerのバージョンに対応するJREバージョンをインストールします。
+
