...
[role="lead"]
工場出荷時に事前設定されていないストレッチ MetroCluster 構成では、コントローラおよびシャーシのコンポーネントの HA の状態が mcc-2n に設定されていて、適切にブートできることを確認する必要があります。工場出荷状態のシステムでは事前に設定されているため、検証は不要です。
...
+
コントローラモジュールおよびシャーシでは、この値が mcc-2n と表示される必要があります。
...
+
*ha-config modify chassis mcc-2n *.ノードを停止します。。ノードが LOADER プロンプトに戻るまで待ちます。
