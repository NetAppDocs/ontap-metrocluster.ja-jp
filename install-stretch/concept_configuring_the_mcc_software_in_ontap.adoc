...
| ノード | ポート | IP アドレス | ネットワークマスク | デフォルトゲートウェイ 
 ...
 a| 
ノード 1 ：このガイドで使用している例： controller_A_1
 a| 
 a| 
 a| 
 a| 
...
| ノード | ポート | IP アドレス | ネットワークマスク | デフォルトゲートウェイ 
 ...
 a| 
ノード 1 ：このガイドで使用している例： controller_B_1
 a| 
 a| 
 a| 
 a| 
...
link:concept_preparing_for_the_mcc_installation.html["専用のポートを使用する場合の考慮事項"]

link:concept_preparing_for_the_mcc_installation.html["データポートを共有する場合の考慮事項"]
...
+
RO アクセスを必要とする場合、「 User (ro):: 」の後に、「 snmpv3user 」を次の例のように指定します。
