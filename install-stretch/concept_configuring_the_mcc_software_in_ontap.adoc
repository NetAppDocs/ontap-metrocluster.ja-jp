...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[ONTAPにおけるノードおよびクラスタ構成のワークフロー図]
...
. に記載されているネゴシエートスイッチオーバー、修復、スイッチバックの手順を使用し link:../manage/task_perform_switchover_for_tests_or_maintenance.html["スイッチオーバー、修復、スイッチバックを実行"]ます。
