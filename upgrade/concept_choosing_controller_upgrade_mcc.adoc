...
* 注 4：統合システム（ディスクとコントローラが同じシャーシ内にある）をアップグレードする場合は、既存のシャーシとディスクを保持しながらコントローラ モジュールを交換する必要があります。
...
image:../media/manual-upgrade-combination-group-2-a90-fas90-updated.png["に、グループ2プラットフォームでサポートされるアップグレードの組み合わせを示します。"]
...
image::../media/metrocluster_fc_upgrade_table_aff_fas.png[MetroCluster FCコントローラのアップグレードでサポートされるプラットフォームの組み合わせ]
