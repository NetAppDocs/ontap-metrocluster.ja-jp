...
|===
+
キーマネージャの種類に応じて、最初の起動メニューのプロンプトでオプション「`10`」またはオプション「`11`」を選択してから、オプション `6`を選択してリカバリ手順を実行します。ノードを完全に起動するには、オプション「`1`」（通常起動）に続けてリカバリ手順を繰り返す必要がある場合があります。

. 交換したノードがブートするまで待ちます。
+
いずれかのノードがテイクオーバーモードの場合は、「 storage failover giveback 」コマンドを使用してギブバックを実行します。

. 管理SVMで暗号化が有効になっている場合は、キー管理構成に応じた適切なコマンドを使用してキーをリストアします。
+
|===
