...
使用できません
 a| 
`* ifconfig e0M -addr=__ filer_addr_-mask=__ netmask __ -gw=__ gateway_--dns=__ dns_addr_-domain=_dns_domain__*`
...
** image.tgz ファイルの URL を求めるプロンプトが表示されたら、次のようにパスを入力する必要があります： +`*http://__path_to_the_web-accessible_directory__/image.tgz*`
...
+
。 link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-ssg/home.html["『 Software Setup Guide 』を参照して"] 詳細については、お使いのバージョンの ONTAP のを参照してください。
