...
---
= スイッチオーバーとスイッチバックを使用して MetroCluster IP 構成の AFF A700 から AFF A900 にコントローラをアップグレード（ ONTAP 9.10.1 以降）
:allow-uri-read: 
...
.このタスクについて
* コントローラで ONTAP 9.10.1 以降が実行されている必要があります。
* この手順環境 AFF A700 コントローラモジュールは、 MetroCluster IP 構成で AFF A900 にアップグレードします。
+
|===
...
+
MetroCluster A700 および AFF A900 コントローラを使用した AFF 構成の運用は、このメンテナンス作業以外ではサポートされません。
...
AFF A900 の MetroCluster 設定では、スロット 5 と 7 にある DR カードの各ポートのいずれかを使用します。アップグレードを開始する前に、 AFF A700 のスロット 7 にカードがある場合は、それらをクラスタのすべてのノード用の他のスロットに移動する必要があります。



=== コントローラをアップグレードする前に、 MetroCluster スイッチの RCF ファイルを更新してください

AFF A700 上の MetroCluster IP 構成では、 VLAN は使用されません。AFF A900 の MetroCluster 設定では、 VLAN を使用します。そのため、 AFF A700 から AFF A900 にアップグレードする際に、 RCF ファイルを変更する必要があります。
...
+
** リンク： ./install-ip/task_switch_config_Broadcom.html# reset-the Broadcom-ip-switch-to-factory-defaults # reset-the Broadcom-ip-switch-to-factory-defaults [Broadcom IP switch を工場出荷時のデフォルトにリセット ]
** リンク： ./install-ip/task_switch_config_Broadcom.html# reset-the Broadcom-ip-switch-to-factory-defaults # reset-the -Cisco-ip-switch-to-factory-defaults [ シスコ IP スイッチを工場出荷時のデフォルトにリセットする ]
...
+
** リンク： ./install-ip/task_switch _config_Broadcom.html# reset-the Broadcom-ip-switch-to-factory-defaults # downloading-& installing-the Broadcom-RCF files [ Broadcom RCF ファイルのダウンロードとインストール ]
** リンク： ./install-ip/task_switch_config_Broadcom.html# reset-the Broadcom-ip-switch-to-factory-defaults # downloading-& installing-the -cisco-ip-rcf-files [ ダウンロードしてインストールする Cisco IP RCF ファイル ]
...
AFF A700 から AFF A900 にアップグレードする場合、データネットワークポート、 FCP SAN アダプタポート、および SAS ストレージポートと NVMe ストレージポートを変更する必要はありません。データ LIF は、アップグレード中もアップグレード後もそのままの状態になります。そのため、古いノードのネットワークポートを新しいノードにマッピングする必要はありません。
...
Tiebreaker
 a| 
link:install-stretch/concept_considerations_when_removing_mcc_configurations.html["MetroCluster 設定の削除"] MetroCluster Tiebreaker インストールおよび設定コンテンツで、を参照してください
...
== AFF A700 プラットフォームコントローラモジュールと NVS を取り外します
...
メモ： AFF A700 NVS モジュール NVS モジュールはスロット 6 にあり、システム内の他のモジュールと比較して高さが 2 倍になっています。
...
. AFF A700 NVS のコアダンプデバイスとして使用するアドオンモジュールを使用している場合は、それらを AFF A900 NVS に転送しないでください。AFF A700 コントローラモジュールと NVS から AFF A900 にパーツを転送しないでください。
...
このアップグレードキットに含まれていた AFF A900 NVS とコントローラモジュールを site_B の両方のノードにインストールする必要がありますコアダンプデバイスを AFF A700 NVS モジュールから AFF A900 NVS モジュールに移動しないでください。
...
次の手順を使用して、 site_B の両方のノードのスロット 6 に AFF A900 NVS を取り付けます
...
+
「 <path_the_web-accessible_directory> 」は、「 <ONTAP_version>_image.tgz 」をダウンロードした場所を指します <<step2-download-software,手順 2>>。

+

NOTE: トランクを中断しないでください。

. AFF A900 コントローラモジュールで node_B_1 がブートするまで待ち、次のようにブートメニューオプションを表示します。
+
[listing]
----
...
. 手順を続行するかどうかを確認するメッセージが表示されたら 'y' と入力し ' パッケージの入力を求められたら 'URL:http://<web_server_ip/path_to_web-accessible_directory>/<ontap_version>_image.tgz` と入力します
...
. ブート・メニューからオプション「 5 」を選択して、保守モードに切り替えます。ノードが保守モードで停止し ' コマンド・プロンプト *> が表示されるまで ' プロンプトに yes と表示されます回答
