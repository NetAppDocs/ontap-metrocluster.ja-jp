...
image::../media/workflow_ip_upgrade.png[MetroCluster IP構成におけるコントローラのアップグレードのワークフロー]
...
. MetroClusterノードが管理SVM（クラスタ）用の外部キーマネージャで構成されている場合は、キーサーバ情報を安全な場所にコピーします。オンボードキーマネージャが設定されている場合は、バックアップ情報とパスフレーズを安全な場所にコピーします。
+
[role="tabbed-block"]
====
.オンボード キー マネージャ
--
.. オンボードキーマネージャのバックアップを表示するには、次のコマンドを実行します：
+
「 securitykey manager onboard show-backup 」を参照してください

+
アップグレード手順の後半では、最新のOnboard Key Managerのパスフレーズも必要になります。
...
.外部キー管理ツール
--
後ほど手順の中でキーマネージャの設定を復元する際に必要となる、外部キー管理情報を収集してください。

.. 管理SVMの名前（クラスタ）を表示します。
+
`vserver show -type admin`

+
この例では、管理SVMの名前はcluster_Bです：
...
cluster_B
            admin   -          -          -           -          -
----
.. 管理SVM（クラスタ）に設定されている外部キーマネージャに関連付けられているプライマリキーサーバのアドレスとポート番号を取得します：
+
[NOTE]
====
*** 管理SVMには最大4つのプライマリキーサーバを設定できますが、この手順の後半でキー管理設定をリストアするために必要なのは、プライマリキーサーバの1つのポート番号とアドレスだけです。
*** 選択したプライマリキーサーバーに関連付けられたユーザー名とパスワードがある場合は、復旧プロセスにもその情報が必要になります。
...
----
.. ONTAP 9.16.1以降を実行している場合は、admin SVMのキーストアUUIDを取得します。
+
...
cluster_B::*
----
.. 収集した情報は、後の手順で使用するため、安全な場所に保存してください。これで、以下の情報が揃ったはずです：
+
*** 管理SVMのプライマリキーサーバの1つのポート番号とアドレス
*** 選択したプライマリキーサーバーに関連付けられているユーザー名とパスワード（関連付けられている場合）
*** 管理 SVM のキーストア UUID （ ONTAP 9.16.1 以降で必須）
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["MetroCluster 設定の削除"] _MetroCluster Tiebreaker のインストールと構成コンテンツ_
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[Site_B は非アクティブで、 MetroClusterスイッチオーバー後にアップグレードの準備ができています]
...
+
image::../media/drw_9500_remove_PCM.png[コントローラモジュールをシャーシから取り外す：ロックを解除してスライドさせる]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[シャーシからNVRAMモジュールを取り外す：ロックを解除してタブを引きます]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[NVRAMモジュールをシャーシに取り付ける：位置を合わせてロックする]
...
+
image::../media/drw_9500_remove_PCM.png[コントローラモジュールをシャーシに取り付ける：位置合わせ、固定、ケーブル接続]
...
. MetroClusterシステムの管理SVMがキーマネージャで設定されている場合は、キー管理設定に適したブートメニューオプションを選択してください。
+
|===
...
プロンプトに従って、キーマネージャ構成をリカバリおよびリストアするために次の入力を指定します。

** オンボードキーマネージャのバックアップとパスフレーズ
...
** 管理SVMのプライマリキーサーバの1つのポート番号とアドレス
** 選択したプライマリキーサーバーに関連付けられている場合は、ユーザー名とパスワード
** 管理 SVM のキーストア UUID （ ONTAP 9.16.1 以降で必須）


*注：*管理SVMには最大4つのプライマリキーサーバを設定できますが、リカバリプロセス中に指定できるのは1つのプライマリキーサーバの情報のみです。
...
. ルートボリューム暗号化を使用する場合は、キー管理設定のためにブートメニューオプションを再度選択してください。
+
|===
...
image::../media/mcc_upgrade_cluster_a_switchback_A900.png[MetroClusterスイッチバック後のアップグレードを待機中の Site_A]
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["MetroCluster 構成を追加しています"] _MetroCluster Tiebreaker のインストールと構成_ セクション。
