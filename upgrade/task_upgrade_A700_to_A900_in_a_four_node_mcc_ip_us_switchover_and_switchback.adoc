...
NOTE: に記載されているように、クラスタのすべてのノードのスロット 7 は空にする必要があります  ports from the old nodes to the new nodes セクション。
...
+
「 <path_the_web-accessible_directory> 」は、「 <ONTAP_version>\_image.tgz 」をダウンロードした場所に配置する必要があります ,手順 2。
