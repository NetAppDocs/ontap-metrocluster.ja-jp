...
サポートされるプラットフォームのアップグレードの組み合わせについては、のMetroCluster FCアップグレードの表を参照してください link:concept_choosing_controller_upgrade_mcc.html#metrocluster-fc-controller-upgrades["コントローラのアップグレード手順 を選択します"]。
...
. の手順を使用します http://["MetroCluster 構成を追加しています"^] MetroCluster Tiebreaker インストールおよび設定 _ 。
