...
+
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["MetroCluster 構成を削除"]
...
image::../media/mcc_upgrade_cluster_a_in_switchover.png[Site_B は非アクティブで、 MetroClusterスイッチオーバー後にアップグレードの準備ができています]
...
. ルートボリューム暗号化を使用する場合、使用している ONTAP のバージョンに応じて、ブートメニューオプションを選択するか、ブートメニューコマンドを再度実行してキー管理の設定を行ってください。
+
[role="tabbed-block"]
====
.ONTAP 9.8以降
