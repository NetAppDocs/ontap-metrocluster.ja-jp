...
ネットワークが新しいコントローラで正常に再開されるようにするには、LIFの配置が正しいことを確認し、古いコントローラのネットワーク設定を削除します。
...
CAUTION: 新旧のコントローラに共通のポートがない場合は、データLIFを変更する必要はありません。この手順を省略して、に直接進みます<<upgrades_assisted_without_matching_ports,手順 4>>。
