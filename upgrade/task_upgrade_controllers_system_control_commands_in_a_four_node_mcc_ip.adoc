...
この手順 では、FAS8200からFAS9500へのアップグレードと、AFF A300からAFF A900へのコントローラのアップグレードがサポートされます。
...
ONTAP メディエーターがインストールされている場合は、事前確認の開始前に自動的に検出されて削除されます。削除を確認するには、ユーザ名とパスワードの入力を求められます。アップグレードが完了したとき、または事前確認が失敗した場合、またはアップグレードの続行を中止した場合は、を実行する必要があります link:task_upgrade_controllers_system_control_commands_in_a_four_node_mcc_ip.html#reconfigure-ontap-mediator["ONTAP メディエーターを手動で再設定します"]。
...
+
https://docs.netapp.com/us-en/ontap-systems/index.html["AFF と FAS ドキュメントセンター"^]
