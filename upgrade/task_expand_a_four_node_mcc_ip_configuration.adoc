...
+
次の例では、SVM「cluster01」上のフェイルオーバーグループ「intercluster01」にポート「e0e」と「e0f」を割り当てます：
...
NOTE:  `metrocluster configuration-settings connection show`コマンドの出力は、プラットフォームのモデルによって異なる場合があります。例えば、一部のプラットフォームでは、HA パートナーの行が出力に表示されない場合があります。
