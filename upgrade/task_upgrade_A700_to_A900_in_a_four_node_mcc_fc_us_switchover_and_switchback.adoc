...
. を繰り返してファブリック B のスイッチ（スイッチ 2 と 4 ）を更新します ,手順 2 終了： ,手順 5。
...
|===
. ノード 1 でネットブートを実行します http://<web_server_ip/path_to_web_accessible_directory>/netboot/kernel`[]「 <path_the_web-accessible_directory> 」は、「 <ONTAP_version>_image.tgz 」をダウンロードした場所を指します ,手順 2。
+
