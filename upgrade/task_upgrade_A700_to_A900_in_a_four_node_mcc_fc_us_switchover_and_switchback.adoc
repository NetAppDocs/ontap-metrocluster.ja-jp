...
---
= スイッチオーバーとスイッチバックを使用してMetroCluster FC構成のAFF A700 / A900からFAS9x00にコントローラをアップグレードする（ONTAP 9.10.1以降）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
このタスクを完了すると、 site_A がアクティブになり、両方のサイトにデータが提供されます。site_B は非アクティブで、次の図のようにアップグレードプロセスを開始する準備ができています。
...
== site_B の AFF A700 コントローラモジュールと NVS を取り外します
