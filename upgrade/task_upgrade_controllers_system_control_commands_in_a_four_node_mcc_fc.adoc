...
|===
+
リカバリ手順を実行するには ' キー・マネージャの設定に応じてオプション 10 またはオプション 11 を選択し ' ブート・メニュー・プロンプトでオプション 6 を選択しますノードを完全にブートするには ' リカバリ手順をオプション 1' （通常のブート）の続きで実行する必要がある場合があります
...
+
新しいノード（サイト B ）がサイト A から * Waiting for switchback 状態 * になっていることを確認します
