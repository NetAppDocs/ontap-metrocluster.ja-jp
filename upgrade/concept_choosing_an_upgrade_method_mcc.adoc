...
9.8
 a| 
xref:task_upgrade_controllers_in_a_four_node_fc_mcc_us_switchover_and_switchback_mcc_fc_4n_cu.adoc[スイッチオーバーとスイッチバックを使用して MetroCluster FC 構成のコントローラをアップグレードする]
...
9.8
 a| 
xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[スイッチオーバーとスイッチバックを使用した MetroCluster IP 構成でのコントローラのアップグレード（ ONTAP 9.8 以降）]
...
9.6 以降
 a| 
xref:task_refresh_4n_mcc_fc.adoc[4 ノード MetroCluster FC 構成を更新しています]
...
9.8
 a| 
xref:task_refresh_4n_mcc_ip.adoc[4 ノード MetroCluster IP 構成の更新（ ONTAP 9.8 以降）]
