...
NOTE: これらの手順は、移行およびシステム更新のワークフロー中に使用します。これらのワークフローの一環として、既にこの手順の一部を完了している可能性があります。
...
NOTE: 「 MetroCluster remove-dr-group 」コマンドは、 ONTAP 9.8 以降でのみサポートされます。
...
+
MetroCluster FC 構成では、<<fc_step_7_remove_nodes,古いDRグループノードを削除する>>の手順に進みます。
...
.. 削除対象となる各DRグループノード上のpool1ディスクの所有権を削除します。削除する各ノードに対して、以下の手順を実行してください。このステップで使用される例では、node_A_1が削除されます。
+
... ノードシェルに移動します。
+
`run -node <node_name>`
...
+
ノードが所有するPool1ディスクを含む、すべてのディスクが表示されます。以下の例は、ディスク0n.4とそのパーティションがPool1に割り当てられている場合の出力の一部を示しています。出力は設定によって異なります。
...
... 各pool1ディスクのディスク所有権を削除します。
+
構成にパーティション化されたディスクが含まれている場合は、まず各パーティション化されたディスクの所有権を削除し、次にコンテナディスクの所有権を削除します：

+
`disk remove_ownership <disk_name>`

+
次の例では、コンテナディスク `0n.4`の所有権が削除される前に、パーティション化されたディスクの所有権が削除されます：
...
. [[fc_step_7_remove_nodes]]古い DR グループのノードを削除します。
+
各クラスターでこの手順を実行します。
...
.関連情報
この手順で使用されるコマンドの詳細については、link:https://docs.netapp.com/us-en/ontap-cli/["ONTAPコマンド リファレンス"^]を参照してください。
