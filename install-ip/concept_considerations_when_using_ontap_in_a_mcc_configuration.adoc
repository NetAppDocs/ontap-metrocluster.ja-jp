...
FabricPool の使用に関する一般的な情報については、を参照してください https://["ディスクと階層（アグリゲート）の管理"^]。
...
NOTE: アグリゲートを設定する前に、の「 MetroCluster 構成での FabricPool 用のオブジェクトストアのセットアップ」の説明に従ってオブジェクトストアを設定する必要があります https://["ディスクおよびアグリゲートの管理"^]。
...
. の説明に従って、アグリゲートを含む FabricPool ミラーを作成します https://["ディスクおよびアグリゲートの管理"^]
+
...
の「 SVM 設定のレプリケート」を参照してください http://["データ保護"^] SVM DR 関係の設定の詳細については、を参照してください。
