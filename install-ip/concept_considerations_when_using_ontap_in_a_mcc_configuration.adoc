...
FabricPool の使用に関する一般的な情報については、 _Disks and aggregates management_ を参照してください。
...
NOTE: アグリゲートを設定する前に、ディスクとアグリゲートの管理 _ の「 MetroCluster 構成での FabricPool のオブジェクトストアのセットアップ」の説明に従ってオブジェクトストアを設定する必要があります。
...
. ディスクとアグリゲートの管理 _ の説明に従って、アグリゲートを含む FabricPool ミラーを作成します。
+
https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-psmg/home.html["ディスクおよびアグリゲートの管理"]
