...
.このタスクについて
イーサネットスイッチヘルスモニター（CSHM）は、ネットワークスイッチの動作状態を正常に保ち、デバッグ目的でスイッチログを収集する役割を担います。この手順では、収集の設定、詳細な *サポート* ログの要求、および AutoSupport によって収集される *定期* データの時間ごとの収集の有効化のプロセスについて説明します。
