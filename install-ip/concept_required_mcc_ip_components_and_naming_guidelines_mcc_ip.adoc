...
[role="lead"]
MetroCluster IP 構成に必要な、サポートされているハードウェアおよびソフトウェア コンポーネントを特定します。ドキュメントの例でコンポーネントに使用されている命名規則を確認します。
...
*注：* 25 GbE の速度を実現するには、MAM1Q00A-QSA28 トランシーバーアダプタが必要です。
...
NOTE: アグリゲート名は、MetroClusterサイト全体で一意である必要があります。つまり、サイト A とサイト B に同じ名前を持つ 2 つの異なるデータ アグリゲートを持つことはできません。
