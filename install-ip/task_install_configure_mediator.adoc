...
最新バージョンの 8.0 （ CentOS 8.0.1905 ）コアはに搭載されているためです https://vault.centos.org["CentOS Vault の場合"]必要なカーネルモジュールをコンパイルするためには、対応する kernel-devel パッケージへのアクセスを提供する必要があります。
