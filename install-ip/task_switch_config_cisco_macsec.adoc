...
summary: 必要に応じて、サイト間を走るWAN ISLポートでのみMACsec暗号化を設定してください。MACsec暗号化を使用する場合は、適切なRCFファイルを適用した後に設定を行ってください。 
---
= MetroCluster IPサイトのCisco 9336Cスイッチ上でMACsec暗号化を設定する（オプション）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
必要に応じて、サイト間を走るWAN ISLポートでMACsec暗号化を設定できます。MACsec暗号化を使用する場合は、適切なRCFファイルを適用した後で設定を行う必要があります。

[NOTE]
====
* MACsec暗号化は、サイト間で実行されるWAN ISLポートにのみ適用できます。
* Cisco 9336Cスイッチを設定する際、MACsec暗号化の設定はオプションです。
