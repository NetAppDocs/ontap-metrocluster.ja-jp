...
* MetroCluster スイッチオーバーコマンドで警告プロンプトが表示されます。
+
...
+
... すべてのノードが構成済みの状態で通常モードであることを確認します MetroCluster node show
+
[listing]
----
...
----
... スイッチオーバー操作を開始します MetroCluster switchover
+
[listing]
----
...
----
... ローカルクラスタが構成済みステートで、スイッチオーバーモードになっていることを確認します。 MetroCluster node show
+
[listing]
----
cluster_A::>  metrocluster node show
...
Remote: cluster_B               not-reachable          -
              configured             normal
----
... スイッチオーバー操作が成功したことを確認します MetroCluster operation show
+
[listing]
----
...
. ONTAP 9.4 以前を実行しているシステムの場合は、データアグリゲートを修復します。「 MetroCluster heal aggregates 」
+
...
----
. ONTAP 9.4 以前を実行しているシステムの場合は、ルートアグリゲートを修復します。「 MetroCluster heal root-aggregates 」
+
この手順は、次の構成で必要です。
...
----
+
何らかの理由で自動修復処理が失敗問題した場合は、 ONTAP 9.5 より前のバージョンの ONTAP と同様に、 MetroCluster の修復コマンドを手動で実行する必要があります。修復のステータスを監視し、障害の原因を特定するには、 lun operation show コマンドおよび MetroCluster operation history show -instance コマンドを使用 MetroCluster します。
